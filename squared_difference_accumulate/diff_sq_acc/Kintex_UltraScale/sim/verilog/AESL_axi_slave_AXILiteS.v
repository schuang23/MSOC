...
//------------------------Parameter----------------------
`define TV_IN_a_V "../tv/cdatafile/c.diff_sq_acc.autotvin_a_V.dat"
`define TV_IN_b_V "../tv/cdatafile/c.diff_sq_acc.autotvin_b_V.dat"
`define TV_OUT_dout_V "../tv/rtldatafile/rtl.diff_sq_acc.autotvout_dout_V.dat"
parameter ADDR_WIDTH = 7;
parameter DATA_WIDTH = 32;
parameter a_V_DEPTH = 10;
reg [31 : 0] a_V_OPERATE_DEPTH = 0;
parameter a_V_c_bitwidth = 16;
parameter b_V_DEPTH = 10;
reg [31 : 0] b_V_OPERATE_DEPTH = 0;
parameter b_V_c_bitwidth = 16;
parameter dout_V_DEPTH = 1;
reg [31 : 0] dout_V_OPERATE_DEPTH = 0;
parameter dout_V_c_bitwidth = 48;
parameter START_ADDR = 0;
parameter diff_sq_acc_continue_addr = 0;
parameter diff_sq_acc_auto_start_addr = 0;
parameter a_V_data_in_addr = 32;
parameter b_V_data_in_addr = 64;
parameter dout_V_data_out_addr = 96;
parameter dout_V_valid_out_addr = 104;
...
reg [dout_V_c_bitwidth - 1 : 0] mem_dout_V [dout_V_DEPTH - 1 : 0];
reg dout_V_read_data_finish;
...
//read dout_V reg
reg [31 : 0] read_dout_V_count = 0;
reg read_dout_V_run_flag = 0;
reg read_one_dout_V_data_done = 0;
...
assign TRAN_AXILiteS_read_data_finish = 1 & dout_V_read_data_finish;
assign TRAN_AXILiteS_write_data_finish = 1 & a_V_write_data_finish & b_V_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        dout_V_read_data_finish <= 0;
        read_dout_V_run_flag <= 0; 
        read_dout_V_count = 0;
        count_operate_depth_by_bitwidth_and_depth (dout_V_c_bitwidth, dout_V_DEPTH, dout_V_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_AXILiteS_start_in === 1) begin
            read_dout_V_run_flag = 1; 
        end
        if (TRAN_AXILiteS_transaction_done_in === 1) begin
            dout_V_read_data_finish <= 0;
            read_dout_V_count = 0; 
        end
        if (read_one_dout_V_data_done === 1) begin
            read_dout_V_count = read_dout_V_count + 1;
            if (read_dout_V_count == dout_V_OPERATE_DEPTH) begin
                read_dout_V_run_flag <= 0; 
                dout_V_read_data_finish <= 1;
            end
        end
    end
end

initial begin : read_dout_V
    integer read_dout_V_resp;
    integer process_num;
    integer get_vld;
    integer four_byte_num;
    integer c_bitwidth;
    ...
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = dout_V_c_bitwidth;
    process_num = 4;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_4_finish <= 0;
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            if (read_dout_V_run_flag === 1) begin
                process_busy = 1;
                get_vld = 0;
                //read dout_V vld
                read (dout_V_valid_out_addr, RDATA_reg, read_dout_V_resp);
                ...
                if (get_vld == 1) begin
                    //read dout_V data 
                    for (i = 0 ; i < four_byte_num ; i = i+1) begin
                        read (dout_V_data_out_addr + read_dout_V_count * four_byte_num * 4 + i * 4, RDATA_reg, read_dout_V_resp);
                        if (dout_V_c_bitwidth < 32) begin
                            mem_dout_V[read_dout_V_count] <= RDATA_reg;
                        end
                        else begin
                            for (j=0 ; j < 32 ; j = j + 1) begin
                                if (i*32 + j < dout_V_c_bitwidth) begin
                                    mem_dout_V[read_dout_V_count][i*32 + j] <= RDATA_reg[j];
                                end
                            end
                        end
                    end
                    
                    read_one_dout_V_data_done <= 1;
                    @(posedge clk);
                    read_one_dout_V_data_done <= 0;
                end    
                process_busy = 0;
            end    
            process_4_finish <= 1;
...
initial begin : write_dout_V_file_proc 
  integer fp; 
  integer factor; 
  integer transaction_idx; 
  reg [dout_V_c_bitwidth - 1 : 0] mem_tmp; 
  reg [ 100*8 : 1] str;
  integer i; 
  transaction_idx = 0; 
  count_seperate_factor_by_bitwidth (dout_V_c_bitwidth , factor);
  while(1) begin 
      @(posedge clk);
      while (dout_V_read_data_finish !== 1) begin
          @(posedge clk);
      end
      # 0.1;
      fp = $fopen(`TV_OUT_dout_V, "a"); 
      if(fp == 0) begin       // Failed to open file 
          $display("Failed to open file \"%s\"!", `TV_OUT_dout_V); 
          $finish; 
      end 
      $fdisplay(fp, "[[transaction]] %d", transaction_idx);
      for (i = 0; i < (dout_V_DEPTH - dout_V_DEPTH % factor); i = i + 1) begin
          if (factor == 4) begin
              if (i%factor == 0) begin
                  mem_tmp = mem_dout_V[i/factor][7:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = mem_dout_V[i/factor][15:8];
              end
              if (i%factor == 2) begin
                  mem_tmp = mem_dout_V[i/factor][23:16];
              end
              if (i%factor == 3) begin
                  mem_tmp = mem_dout_V[i/factor][31:24];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 2) begin
              if (i%factor == 0) begin
                  mem_tmp = mem_dout_V[i/factor][15:0];
              end
              if (i%factor == 1) begin
                  mem_tmp = mem_dout_V[i/factor][31:16];
              end
              $fdisplay(fp,"0x%x",mem_tmp);
          end
          if (factor == 1) begin
              $fdisplay(fp,"0x%x",mem_dout_V[i]);
          end
      end 
      if (factor == 4) begin
          if ((dout_V_DEPTH - 1) % factor == 2) begin
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][15:8]);
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][23:16]);
          end
          if ((dout_V_DEPTH - 1) % factor == 1) begin
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][7:0]);
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][15:8]);
          end
          if ((dout_V_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][7:0]);
          end
      end
      if (factor == 2) begin
          if ((dout_V_DEPTH - 1) % factor == 0) begin
              $fdisplay(fp,"0x%x",mem_dout_V[dout_V_DEPTH / factor][15:0]);
          end
      end
