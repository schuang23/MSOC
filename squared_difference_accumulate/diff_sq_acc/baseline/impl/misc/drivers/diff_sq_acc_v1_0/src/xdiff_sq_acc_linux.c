...
typedef struct {
    ...
    XDiff_sq_acc_uio_map maps[ MAX_UIO_MAPS ];
} XDiff_sq_acc_uio_info;

/***************** Variable Definitions **************************************/
static XDiff_sq_acc_uio_info uio_info;
...
static int uio_info_read_name(XDiff_sq_acc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDiff_sq_acc_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDiff_sq_acc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDiff_sq_acc_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDiff_sq_acc_Initialize(XDiff_sq_acc *InstancePtr, const char* InstanceName) {
	XDiff_sq_acc_uio_info *InfoPtr = &uio_info;
